...
t2m = cclm_template.clone("T_2M_AV", contour = True, color_map = 'YlOrRd', transform_variable = convert_K2C, task_name="seasonal_mean")
...
for season in seasons:
    ...
    plot_configs[season] = [t2m.clone(), 
                            t2m.clone(task_name="process_reference", title = "T_2M_AV-" + season + "-reference", lon_name="longitude", lat_name="latitude"),
                            cclm_template.clone("T_2M_AV", contour = True, task_name="calculate_anomalies", min_value = -6.5, max_value = 6.5, delta_value = 1.0, title = "T_2M_AV-" + season + "-anomaly", color_map = 'seismic'),
                            rain.clone(),
