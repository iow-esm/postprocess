...
for kind in kinds:
    ...
    for var in variables.keys():
        ...
        for i, dat in enumerate(data.keys()):
            ...
            for j, operator in enumerate(operators):
                ...
                for k, model in enumerate(model_dirs.keys()):
                    ...
                    diagram.add_sample(dm, label=model)
        ...
        table = axs.table(cellText=[['']*len(operators)],
                colLabels=[better_operator_name(o[1:]) for o in operators],
                loc='center',
...
f = open(pwd+"/"+results_dir+"/get_cost_function.py", "w")
f.write(script)
f.close()
...
convertpy2ipynb.convertpy2ipynb(pwd+"/"+results_dir+"/get_cost_function.py")